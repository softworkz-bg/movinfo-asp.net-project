...
function addAllActorsEdited(event) {
    var resultedActorsOldAndNew = actorArray.join(',') + ',' + originalActorsArray.join(',');
    var resultedActorsOldAndNewTrimmed = resultedActorsOldAndNew.replace(/(^,)|(,$)/g, "");
    $(".actors-form-submit").val(resultedActorsOldAndNewTrimmed);
}
